...
def _build_long_instruction():
    return u'[enter]:Select, [q]uit, [b]rowse'
...
class RefactorList:
    ...
    def show(self):
        ...
        while True:
            ...
            prompt = inquirer.select(message='Select unit', choices=units,
    ...
    def show_snippet(self, choice: dict):
        ...
        with open(file_path) as file:
            code = file.read()
        snippet = get_location_range(code, choice['measurement']['start'],
                                     choice['measurement']['end'])
        click.echo(snippet)
        inquirer.text('[Press ENTER to continue]', raise_keyboard_interrupt=False).execute()
