...
def test_format_report_unit():
    report_unit = ReportUnit('foo.py', Measurement('bar()', Location(10, 1), Location(11, 1), 1))

    assert format_report_unit(report_unit).markup == '[[green]  1[/green]] bar()'

    report_unit = ReportUnit('foo.py', Measurement('bar()', Location(10, 1), Location(30, 1), 20))

    assert format_report_unit(report_unit).markup == '[[yellow] 20[/yellow]] bar()'

    report_unit = ReportUnit('foo.py', Measurement('bar()', Location(10, 1), Location(80, 1), 70))
