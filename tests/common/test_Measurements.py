...
def test_to_json():
    ...
    expected += '      ".": {\n'
    expected += '        "entries": [\n'
    expected += '          {"name": "foo.py", "loc": 10, "risk_categories": [10, 0, 0, 0]}\n'
    expected += '        ]\n'
    expected += '      }\n'
    expected += '    },\n'
...
def test_to_json_multiple():
    ...
    expected = '{"uuid": "' + report.uuid + '", "codebase": {"tree": {".": {"entries": [{"name": "foo.py", "loc": 10, "risk_categories": [10, 0, 0, 0]}, ' + \
               '{"name": "bar.py", "loc": 10, "risk_categories": [10, 0, 0, 0]}]}}, ' + \
