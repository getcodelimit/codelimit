...
def test_to_json():
    codebase = Codebase()
    codebase.add_file('foo.py', [SourceMeasurement('bar()', 10, 10)])
    codebase.aggregate()
    ...
    expected += '      "foo.py": [\n'
    expected += '        {"unit_name": "bar()", "start_line": 10, "value": 10}\n'
    expected += '      ]\n'
    expected += '    }\n'
    expected += '  }\n'
    expected += '}\n'
...
def test_to_json_multiple():
    codebase = Codebase()
    codebase.add_file('foo.py', [SourceMeasurement('bar()', 10, 10)])
    codebase.add_file('bar.py', [SourceMeasurement('foo()', 20, 10)])
    codebase.aggregate()
    report = Report(codebase)
    ...
    expected = '{"uuid": "' + report.uuid + '", "codebase": {"tree": {"./": {"entries": [{"name": "foo.py", "profile": [10, 0, 0, 0]}, ' + \
               ...
               '"measurements": {"foo.py": [{"unit_name": "bar()", "start_line": 10, "value": 10}], "bar.py": ' + \
               '[{"unit_name": "foo()", "start_line": 20, "value": 10}]}}}'
    assert serializer.to_json() == expected


def test_all():
    measurements = Codebase()
    measurements.add_file('foo.py', [SourceMeasurement('bar()', 10, 10)])
    measurements.add_file('bar.py', [SourceMeasurement('foo()', 20, 10), SourceMeasurement('spam()', 30, 10)])
